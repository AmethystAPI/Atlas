...
void Minimap::Render(MinecraftUIRenderContext& ctx)
{
	...
	ctx.fillRectangle(rect, mce::Color::BLACK, 0.67);
	...
	for (int x = -mRenderDistance - 1; x <= mRenderDistance + 1; x++) {
		for (int z = -mRenderDistance - 1; z <= mRenderDistance + 1; z++) {
			...
			// Calculate player's position within their current chunk (0-16)
			float playerChunkOffsetX = playerPos->x - (playerChunkPos.x * 16);
			float playerChunkOffsetZ = playerPos->z - (playerChunkPos.z * 16);

			// Calculate screen position based on relative chunk offset
			float xChunkTranslation = ((x * 16) - playerChunkOffsetX + (mRenderDistance * 16)) * unitsPerBlock;
			float zChunkTranslation = ((z * 16) - playerChunkOffsetZ + (mRenderDistance * 16)) * unitsPerBlock;
